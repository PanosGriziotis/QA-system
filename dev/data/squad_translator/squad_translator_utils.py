...
def de_tokenize(text, lang):
    ...
    if lang == 'en':
        ...
        text_detok = detokenizer_el.detokenize(text, return_str=True)
        return text_detok
...
def remove_extra_punct(source, translation):
    #print (f"REMOVE_EXTRA_PUNCT:\t{translation}")
    periods_commas = '.,;:'
    brackets = [['(', ')'], ['[', ']'], ['{', '}']]
    exclamation = '!'
    quotation = '"'
...
def remove_extra_text(source, translation, lang='el'):
    #TODO: this method trimms a lot of correct answers.

    with open ("translated_answers.txt", "a", encoding="utf-8") as writer:
        writer.write (f"\nno trim:\t {translation}")
...
def post_process_answers_translated(source, translation):
    ...
    # Post-process the answer translated
        ...
        print (f"AFTER_REMOVED_EXTRA_TEXT:\t{translation}")
        translation = remove_extra_punct(source, translation)
        print (f"AFTER_REMOVED_EXTRA_PUNCT:\t{translation}")
...
# example:
# text_raw = hello Panos
# text_tok = hello Panos
# tok2char_map = {0: 0, 1: 6}
...
# target_text = πως είσαι
# tok_alignment = 0-0 1-1 2-1
# sr2tran_alignment_char_min_tran_index = {0: 0, 4: 4, 8: 4}
...
def extract_answer_translated(answer, answer_translated, context, context_translated, context_alignment_tok,
    ...
    # No answer translated found
        ...
        print (f"no answer translated found for the following answer: {answer}")
        answer_translated = ''
        answer_translated_start = -1

    # Post-process if the answer is not empty
    if answer_translated:
        print (f"ANSWER BEFORE PROCESSING: {answer_translated}")
...
# Translate via google_api script
def translate(source_sentences, file, output_dir):
    
    # TODO remove from source file the empty lines

    filename = os.path.basename(file)
    source_filename = os.path.join(output_dir, '{}_source_translate.txt'.format(filename))
    with open(source_filename, 'w') as sf:
        sf.writelines('\n'.join(str(s) for s in source_sentences))

    translation_filename = os.path.join(output_dir, '{}_target_translated.txt'.format(filename))
    
    # call google api and translate sentences
    google_api = GoogleApi()
    #google_api.upload_file_to_bucket(source_filename)
    #google_api.batch_translate_text(os.path.basename(source_filename), 'results/')
    google_api.download_translated_file (translation_filename)
...
def compute_alignment(source_sentences, source_lang, translated_sentences, target_lang):
    ...
    alignments = []
    myaligner = SentenceAligner(model="bert-base-multilingual-cased", token_type="bpe", matching_methods="mai", device='cuda')

    for sentence_pair in tqdm (zip (source_sentences,translated_sentences), total=len(source_sentences)):
        alignment = myaligner.get_word_aligns (sentence_pair[0], sentence_pair[1])['itermax']
