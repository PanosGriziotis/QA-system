...
DENSE_RETRIEVER_MODEL= os.environ["DENSE_RETRIEVER_MODEL"] if "DENSE_RETRIEVER_MODEL" in os.environ else "panosgriz/covid-el-paraphrase-multilingual-MiniLM-L12-v2"
...
EXTRACTIVE_READER_MODEL = os.environ["EXTRACTIVE_READER_MODEL"] if "EXTRACTIVE_READER_MODEL" in os.environ else "panosgriz/mdeberta-v3-base-squad2-covid-el-small"
...
def init_rag_pipeline (use_gpu:bool=False):
    ...
    p.add_node(component=generator, name="GenerativeReader", inputs=["Reranker"])
    p.add_node(component=responder, name="Responder", inputs=["GenerativeReader"])
...
def init_extractive_qa_pipeline (use_gpu:bool=True):
    ...
    p.add_node(component=join_documents, name="JoinDocuments", inputs=["BM25Retriever", "DenseRetriever"])
    p.add_node(component=ranker, name="Reranker", inputs=["JoinDocuments"])
    p.add_node(component=reader, name="ExtractiveReader", inputs=["Reranker"])
    p.add_node(component=responder, name="Responder", inputs=["ExtractiveReader"])
